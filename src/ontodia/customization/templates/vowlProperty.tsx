...
export class VowlProperty extends Component<TemplateProps, {}> {
    private getColor(types: string): string {
        if ( types.indexOf('DatatypeProperty') !== -1 ) {
            return '#9c6'; } else if (types.indexOf('ObjectProperty') !== -1) {
            return '#1bf1ee'; } else {
        return undefined; }
    }
    render() {
        ...
        return (
            ...
            <div className={CLASS_NAME}
                style={{backgroundColor: this.getColor(this.props.types), borderColor: 'black'}}>
                    <div className='ontodia-vowl-property-template_body'> 
                        {imgUrl ? (
                            <CrossOriginImage className={`${CLASS_NAME}__picture`}
