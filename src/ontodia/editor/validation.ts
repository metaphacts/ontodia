...
async function processValidationResult(
    ...
    if (editor.validationState.elements.get(e.target.id) === previousElement) {
        editor.setValidationState(ValidationState.setElementErrors(
            editor.validationState, e.target.id, elementErrors,
        ));
    }

    linkErrors.forEach((errors, link) => {
        if (editor.validationState.links.get(link) === previousLink) {
            editor.setValidationState(ValidationState.setLinkErrors(
                editor.validationState, link, errors
            ));
        }
    });
}
